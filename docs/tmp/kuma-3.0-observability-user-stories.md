...
|---------|------|-------|---------|
| **Mesh Operator** | Deploys and manages Kuma control plane and mesh infrastructure | Reliable mesh with clear operational visibility, simple upgrades | Manages Kuma across environments, configures policies, monitors mesh health |
| **Platform Engineer** | Builds internal developer platform on top of Kuma | Standardized observability stack, seamless integration with org-wide tooling (OTel collectors, Grafana, etc.) | Owns observability pipelines, defines standards for service teams |
| **Service Developer** | Develops microservices running inside the mesh | Understand how their service behaves, debug latency/errors quickly | Consumes dashboards and traces, doesn't manage mesh infrastructure |
| **SRE** | Ensures service reliability, manages incidents | Track SLOs, get alerted on degradation, fast root-cause analysis | Uses golden signals, distributed tracing, cross-zone visibility |
...
### 1.4 Remove observability-related ConfigMap reconcilers

**As a** Mesh Operator,
**I want** any ConfigMap reconcilers that were only needed for the legacy `install observability` flow removed,
**so that** the control plane doesn't run unnecessary reconciliation loops.

**Acceptance Criteria:**
- Audit existing ConfigMap reconcilers for observability-specific logic
- Remove any reconcilers/watchers that only served the legacy observability installation
- VIP-related ConfigMap reconciler (unrelated) remains untouched
...
**As a** Platform Engineer using Prometheus Operator,
...
**As a** Platform Engineer running a heterogeneous service mesh (Kuma + Istio sidecars, Zipkin-instrumented services),
...
|----------|------|---------------|
| P0 | Epic 1: Remove `kumactl install observability` | 2.14 (deprecation) + 3.0 (removal) |
